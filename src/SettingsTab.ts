...
export default class SettingsTab extends PluginSettingTab {
	...
	constructor(app: App, plugin: WrapWithShortcut) {
		super(app, plugin);
		this.plugin = plugin;
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Create a new wrapper to wrap the selected text in the custom tags."
			)
			.addButton((cb) => {
			...
			new Setting(containerEl)
				...
				.addExtraButton((bt) => {
					...
					bt.onClick(async () => {
						...
						this.display();
						new Notice(
							"You will need to restart Obsidian to fully remove the Wrapper."
						);
						await this.plugin.saveSettings();
					});
				});
